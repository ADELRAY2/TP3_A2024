...
from exceptions import ValeurHorsLimites
...
class JeuDeCanon:
    ...
    def tirer(self):
        ...
        # Aide:
        # self.angle = float(
        #     self.commandes.angle_entry.get()
        # )  # Récupération de l'angle depuis l'entrée utilisateur
        #
        # self.puissance = float(
        #     self.commandes.puissance_entry.get()
        # )  # Récupération de la puissance depuis l'entrée utilisateur
        try:
         ...
         if not (ANGLE_MIN <= self.angle <= ANGLE_MAX):
             raise ValeurHorsLimites(f"L'angle doit être entre {ANGLE_MIN} et {ANGLE_MAX}.")
         if not (PUISSANCE_MIN <= self.puissance <= PUISSANCE_MAX):
             raise ValeurHorsLimites(
                 f"La puissance doit être entre {PUISSANCE_MIN} et {PUISSANCE_MAX}."
             )
        ...
        except ValeurHorsLimites as e:
              # Affiche un message d'erreur si les entrées ne sont pas valides
              e.affichage_de_lerreur()

    def afficher_message_touche(self):
        """Affiche le message lorsque la cible est touchée."""
        # TODO: À compléter, Fait
        # Aide:
        # - Se documenter sur la méthode `create_text` disponible pour les objets Canvas
        self.message_touche = self.canvas.create_text(
            400, 300, text="Cible touchée!", font=("Helvetica", 20), fill="green"
        )
        self.canvas.after(2000, self.effacer_message_touche)  # Efface le message après 2 secondes
    ...
    def soumettre_score(self, nom, fenetre_parente):
        ...
        # Aide:
        #   - Pensez à utiliser messagebox
        #   - Pensez à utiliser la méthode `self.classement.enregistrer_score`
        if not nom:
            messagebox.showerror("","Le nom ne peut pas être vide.")
            'ok'
